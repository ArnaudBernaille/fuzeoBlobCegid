...
import pandas as pd
from apscheduler.schedulers.blocking import BlockingScheduler # Pour automatisation
sched = BlockingScheduler() # Pour automatisation

def jourToSecond(jour):
    # Entré :  un nombre de jour
    # Sortie : l'équivalent en seconde
    return jour * 24 * 60 * 30



#Ajout d'un décorateur pour lancer le script tous les X secondes
@sched.scheduled_job('interval', seconds=jourToSecond(1))
def timed_job():
    print("lancement du programme" + str(datetime.now()))
    #_________
