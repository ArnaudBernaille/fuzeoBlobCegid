...
L_fichierADL = [] # Contiend la liste des noms des fichier à télécharger
for nom_fc in df_blobCegid['fichier_cible'].unique():
    L_fichierADL.append(df_blobCegid.loc[df_blobCegid['fichier_cible'] == nom_fc].sort_values(by = 'date', ascending = False).iloc[0,0])

print("\n")
print("liste des fichiers que nous allons télécharger :")
for k in L_fichierADL:
    print(k)
...
with open(download_file_path, "wb") as download_file:
    download_file.write(container.download_blob("DB508603/DB508603-cpa-societe;-20210927164757.csv").readall())
